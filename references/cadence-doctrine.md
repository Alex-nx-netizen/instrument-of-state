...
### 示例
- Menxia 出具 verdict 后，coordinator 向用户发一张"批准 / 有条件 / 退回 / 驳回"牌
- B1 批次完成后，Works-Delivery 向用户发一张"go / halt / revise"牌
...
### Examples
- After Menxia issues a verdict, coordinator deals an "approve / conditional / remand / reject" card
- After B1 batch completes, Works-Delivery deals a "go / halt / revise" card
...
### 示例
- Works-Delivery 在执行 B1 的中段写文件，无阻塞，不向用户发任何消息；只在 B1 结束时一次性发牌
- Menxia 正在独立审议中，Zhongshu 与 Works 都不应主动询问进度
...
### Examples
- Works-Delivery mid-execution of B1 writing files with no blockers — emits no message until B1 finishes and then deals once
- Menxia is in independent deliberation — neither Zhongshu nor Works should ask about progress
...
### 示例
- 某 docket 的 scope 纯为文档修订，无代码变更 → `war-operations` 阶段被跳过，记录为 `skip: no-operational-surface`
- 某批次只新增文件、无删除、无改名 → `rollback-rehearsal` 阶段被跳过，记录为 `skip: pure-addition`
- 用户上一条输入已明示"我不需要公开发布" → `rites-protocol` 的 publication 段被跳过，记录为 `skip: user-explicit-opt-out`
...
### Examples
- A docket whose scope is pure doc revision with no code change → `war-operations` stage is skipped with `skip: no-operational-surface`
- A batch that only adds files without deletions or renames → `rollback-rehearsal` stage is skipped with `skip: pure-addition`
- The user explicitly said "no public release needed" → `rites-protocol` publication sub-stage is skipped with `skip: user-explicit-opt-out`
...
- **P1**：核心 gate（Menxia 审议通道、Justice 验收通道）失效 → 插队修复
- **全局影响**：一个公共组件（guard 脚本、hooks.json、workflow-contract.json）故障影响所有 docket → 插队修复
- **用户显式升级**：用户以"紧急 / 先停下 / halt"等关键词明示抢占 → 插队

### 先插队后补手续规则
插队允许跳过常规 gate（如 memorial → Menxia → Works 的顺序），但必须在事后 72 小时内：
1. 补一份 post-hoc memorial，记载为什么插队
2. 补一次 Menxia review（回溯审议）
3. 把插队事件写回 `memory/` 作为 scar

未补手续的插队将触发 `war-operations` 的自检失败并 DENY 后续动作。

### 示例
- `bin/instrument-guard.ps1` 出现 parse error 影响所有 Write 操作 → war-operations 立即热修，事后补 memorial
- 用户输入"halt, guard 行为异常" → 立即暂停所有 in-flight subagent，进入诊断，事后补审议
- secret 出现在 git 日志 → 立即执行 secret 轮换 + 历史清除，事后 Menxia 复审

### 反模式
- **伪插队**：把"我很急"当作插队依据，绕过 Menxia 审议。这是权力分立被破坏的信号，应被 guard 层硬拦截。
- **插队不补**：事后 72h 内不补手续，事实上等同于永久性越权。
- **多重插队**：一次插队未补完又发起下一次插队，形成"永久紧急态"。war-operations 在检测到连续 3 次未补完的插队时应自动熔断。
...
- **P1**: a core gate (Menxia review channel, Justice verification channel) is broken → preempt to repair
...
Preempt may bypass the usual gate sequence (memorial → Menxia → Works), but within 72 hours post-event must:
1. File a post-hoc memorial recording why the preempt happened
2. File a Menxia review (retrospective deliberation)
3. Write the preempt event back to `memory/` as a scar

Preempts without paperwork filed will fail `war-operations` self-check and DENY subsequent actions.

### Examples
- `bin/instrument-guard.ps1` throws a parse error affecting all Write ops → war-operations hot-fixes immediately, files memorial post-hoc
- User says "halt, guard is misbehaving" → pause all in-flight subagents, enter diagnosis, file deliberation post-hoc
- A secret shows up in git log → execute rotation + history scrub immediately, file Menxia review post-hoc

### Anti-patterns
- **Fake preempt**: using "I'm in a hurry" as preempt grounds to bypass Menxia. This is a signal that separation of powers is being breached — should be caught by the guard layer.
- **Paperwork default**: not filing within 72h — effectively a permanent power-grab.
- **Serial preempt**: new preempt started before previous preempt is paperworked — a "permanent emergency" state. War-operations must auto-break after 3 consecutive unpaperworked preempts.
